package com.alibaba.weex.extend.module.widget.actionsheet;
...
public class WXActionSheet extends Fragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        colorMap.put("transparent", Color.TRANSPARENT);
    }
    ...
    private void prepareArguments() {
        ...
        if (bundle != null) {
            ...
            String colorText = bundle.getString(KEY_COLOR);
    ...
    private View makeItem(final String[] item, final int index) {
        TextView textView = createItemBase();
        textView.setText(item[1]);

        if ("2".equals(item[0])) {
            if (colorMap.containsKey(item[2])){
                textView.setTextColor(colorMap.get(item[2]));
            }else {
                textView.setTextColor(COLOR_ITEM_ALERT);
            }
//            textView.setTextColor(COLOR_ITEM_ALERT);
        } else if ("0".equals(item[0])) {
            if (colorMap.containsKey(item[2])){
                textView.setTextColor(colorMap.get(item[2]));
            }else {
                textView.setTextColor(COLOR_ITEM_NORMAL);
...
//            textView.setTextColor(COLOR_ITEM_NORMAL);
        ...
        textView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (actionListener != null) {
                    actionListener.onSelected(index, item[1]);
                    dismiss();
                }
            }
        });
    ...
    public static class Builder {
        ...
        private Bundle parseToBundle() {
            ...
            for (Map<String, Object> map : items) {
                ...
                itemsBundle.add(type + SEPARATOR + message + SEPARATOR + color);
            }
